...
if(document.querySelector('.tree__text')) {
    let treeItem = document.querySelectorAll('.tree__item');

    treeItem.forEach(e => {
        let textTree = e.querySelector('.tree__text');
        let treeInput = e.querySelector('.tree__input');
        textTree.addEventListener('click', (event) => {
            subdivisionFromTreeToTextarea(textTree);
            subdivisionIdFromTreeToInputSubdivisionForm(treeInput);
            event.stopPropagation();
        })
    })
}
function subdivisionIdFromTreeToInputSubdivisionForm(treeInput) {
    let id = treeInput.value;
