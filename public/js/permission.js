...
if(document.querySelector('.table-permission__row')) {
    ...
    function setActiveForms(color) {
        ...
        let formViewPermission = document.getElementById('view-permission');
        ...
        if(color == 'violet') {
            ...
            if(userRole === 'Диспетчер') {
                ...
                activeForms = [formViewPermission, formStoryPermission, formPDFPermission, formApplyPermission, formCreateByPermission, formCancelAgreementPermission];
            }else if(userRole === 'Проверяющий инженер') {
                activeForms = [formViewPermission, formStoryPermission];
            ...
            if(userRole === 'Диспетчер') {
                ...
                let formUnactiveMaskingPermission = document.getElementById('unactivemasking-permission');
                activeForms = [formEditPermission, formUnactiveMaskingPermission, formViewPermission, formStoryPermission, formFinishWork, formOpenPermission, formClosePermission, formStartWork, formActiveMaskingPermission];
            } else if(userRole === 'Автор') {
                let formEditPermission = document.getElementById('edit-permission');
                let formCancelApplyPermission = document.getElementById('cancel-apply-permission');
                let formCreateByPermission = document.getElementById('create-by-permission');
                let formPDFPermission = document.getElementById('pdf-permission');
                activeForms = [formViewPermission, formStoryPermission, formPDFPermission, formEditPermission, formCancelApplyPermission, formCreateByPermission];
            }else if(userRole === 'Проверяющий инженер') {
                ...
                activeForms = [formViewPermission, formStoryPermission, formMaskingPermission, formCheckUnmaskingPermission];
            }else if(userRole === 'Сменный инженер') {
                ...
                activeForms = [formViewPermission, formStoryPermission, formMaskingPermission, formUnmaskingPermission];
            }
        } else if(color == 'green') {
            if(userRole === 'Диспетчер') {
                let formStartWork = document.getElementById('start-work');
                ...
                let formFinishWork = document.getElementById('finish-work');
                let formUnactiveMaskingPermission = document.getElementById('unactivemasking-permission');
                activeForms = [formUnactiveMaskingPermission, formViewPermission, formStartWork, formFinishWork, formStoryPermission, formClosePermission, formPausePermission, formActiveUnaskingPermission, formActiveMaskingPermission];
            } else if(userRole === 'Автор') {
                let formCreateByPermission = document.getElementById('create-by-permission');
                let formPDFPermission = document.getElementById('pdf-permission');
                activeForms = [formViewPermission, formStoryPermission, formCreateByPermission, formPDFPermission];
            }else if(userRole === 'Проверяющий инженер') {
                let formMaskingPermission = document.getElementById('check_masking-permission');
                let formCheckUnmaskingPermission = document.getElementById('check_unmasking-permission');
                activeForms = [formViewPermission, formStoryPermission, formMaskingPermission, formCheckUnmaskingPermission];
            ...
            if(userRole === 'Диспетчер') {
                ...
                let formActiveMaskingPermission = document.getElementById('activemasking-permission');
                let formUnactiveMaskingPermission = document.getElementById('unactivemasking-permission');
                activeForms = [formUnactiveMaskingPermission, formViewPermission, formStoryPermission, formStartWork, formFinishWork, formClosePermission, formOpenPermission, formActiveUnaskingPermission, formActiveMaskingPermission];
            } else if(userRole === 'Автор') {
                let formPDFPermission = document.getElementById('pdf-permission');
                let formCreateByPermission = document.getElementById('create-by-permission');
                activeForms = [formStoryPermission, formViewPermission, formPDFPermission, formCreateByPermission];
            }else if(userRole === 'Проверяющий инженер') {
                let formMaskingPermission = document.getElementById('check_masking-permission');
                let formCheckUnmaskingPermission = document.getElementById('check_unmasking-permission');
                activeForms = [formViewPermission, formStoryPermission, formMaskingPermission, formCheckUnmaskingPermission];
            ...
            if (userRole === 'Диспетчер') {
                ...
                activeForms = [formPausePermission, formViewPermission, formStoryPermission, formActiveUnaskingPermission, completePermission, formFinishWork];
            } else if (userRole === 'Автор') {
                let formPDFPermission = document.getElementById('pdf-permission');
                let formCreateByPermission = document.getElementById('create-by-permission');
                activeForms = [formViewPermission, formStoryPermission, formCreateByPermission, formPDFPermission];
            ...
            if (userRole === 'Диспетчер') {
                ...
                activeForms = [formViewPermission, formStoryPermission, dormRecoveryPermission];
            } else if (userRole === 'Автор') {
                let formCreateByPermission = document.getElementById('create-by-permission');
                activeForms = [formViewPermission, formStoryPermission, formCreateByPermission];
            } else if (userRole === 'Проверяющий инженер') {
                activeForms = [formViewPermission, formStoryPermission];
            } else if (userRole === 'Сменный инженер') {
                activeForms = [formViewPermission, formStoryPermission];
...
if(document.querySelector('.table-permission__background')) {
    ...
    tableWorkColors.forEach(e => {
        ...
        if(e.value === 'aquamarine') {
            delColor(cell);
            cell.classList.add('table-permission__col_aquamarine');
        } else if(e.value === 'wood') {
            delColor(cell);
            cell.classList.add('table-permission__col_wood');
        }
    });
    ...
    tableMaskColors.forEach(e => {
        ...
        if(e.value === 'darkviolet') {
            ...
            delColor(cell);
            cell.classList.add('table-permission__col_red');
        }
    });
}
